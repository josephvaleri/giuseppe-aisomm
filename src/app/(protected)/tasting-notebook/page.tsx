        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {notes.map((note) => (
            <Link 
              ...
              className="rounded-2xl border border-amber-200 bg-white p-4 hover:shadow-lg hover:border-amber-300 transition-all duration-200"
            >
                ...
                <div className="flex">
                  {Array.from({ length: 5 }, (_, i) => (
                    <span
                      key={i}
                      className={`text-lg ${
                        i < Math.floor(Number(note.rating_bottles ?? 0))
                          ? "text-amber-500"
                          : "text-gray-300"
                      }`}
                    >
